...
namespace ChemCloud.Core.Helper
{
	public class IOHelper
	{
		...
		public static void CopyFile(string fileFullPath, string destination, bool isDeleteSourceFile = false, string fileName = "")
		{
            ...
            //}
            var req=WebRequest.Create(fileFullPath);
			...
			try
			{
                ...
                WebClient.DownloadFile(fileFullPath, Path.Combine(destination, fileName));
                WebClient.Dispose();
                //if (isDeleteSourceFile)
                //{
                //    WebClient.UploadDataAsync(fileFullPath, "DELETE", new byte[0]);
