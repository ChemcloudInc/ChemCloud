...
namespace ChemCloud.Web.Framework
{
	public static class SellerPermission
	{
		...
		{
            if (userprivileages.Contains(0) || controllerName.Equals("MessageDetial") || controllerName.Equals("Certification") || controllerName.Equals("Shop"))
			{
				return true;
			}
			return (
		...
		{
			List<ActionPermission> actionPermissions = new List<ActionPermission>();
			IEnumerable<Type> types = ((IEnumerable<Type>)Assembly.Load("ChemCloud.Web").GetTypes()).Where((Type a) => {
				if (a.BaseType == null)
				{
					return false;
				}
