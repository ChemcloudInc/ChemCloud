$(function () {
    ...
    $('#moduleContainer').on('click', 'a.choose-goods', function () {
        ...
        var ids = null;
        if (moduleProducts[moduleIndex]) {//当前模块已选择过产品，则获取所有本模块产品的编号
            ids = [];
...
function addModule() {
    ...
    var html = ' <tr index="' + moduleIndex + '">\
                            ...
                            <td class="td-operate"><span class="btn-a"><a class="choose-goods" index="' + moduleIndex + '">选择产品</a><a class="a-del" index="' + moduleIndex + '">删除</a></span></td>\
                 </tr>';
...
function generateTopicInfo() {
    ...
    $.each(modules, function (i, moduleItem) {
        ...
        if (!moduleProducts[moduleIndex] || moduleProducts[moduleIndex].length == 0)
            throw new Error('“' + moduleInfo.name + '”至少要选择一件产品');
