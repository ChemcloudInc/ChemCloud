...
function generateRoleInfo() {
    ...
    if (role.RoleName == null || role.RoleName == "" || role.RoleName.length >= 100) {
        $.dialog.tips("请确保权限组名长度在1~100个字符之间！");
        return;
    }
    ...
    if ($('form').valid()) {
        try {
            ...
            $.post('./add', { roleJson: objectString }, function (result) {
                loading.close();
                if (result.success) {
                    $.dialog.tips('保存成功', function () { location.href = '/SellerAdmin/Privilege/Management'; });
                }
                else
