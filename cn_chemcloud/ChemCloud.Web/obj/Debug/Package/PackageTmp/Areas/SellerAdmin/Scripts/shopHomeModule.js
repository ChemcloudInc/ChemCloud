...
function bindAddModuleBtnClickEvent() {
    $('#addModule').click(function () {
        var loading = showLoading();
        $.post('AddShopHomeModule', { name: '新产品模块' }, function (result) {
            loading.close();
            if (result.success) {
                var id = result.id;
...
function bindDeleteBtnClickEvent() {
    $('#moduleTable').on('click', 'a[delete]', function () {
        ...
        $.dialog.confirm('您确定要删除产品模块 ' + name + ' 吗？', function () {
            del(id);
        });
    })
}
...
function addNewRow(id, name, productIds) {
    ...
    var html = '<tr moduleId="' + id + '">\
                    ...
                    <td class="td-operate"><span class="btn-a"><a productIds="' + productIds.toString() + '">选择产品</a><a delete>删除</a></span></td>\
                </tr>';
