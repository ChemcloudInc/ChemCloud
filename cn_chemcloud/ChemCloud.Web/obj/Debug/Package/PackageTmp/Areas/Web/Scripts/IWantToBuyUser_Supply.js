...
function span_ModifyFun(obj) {
    ...
    $.ajax({
        type: "POST",
        url: "/IWantToBuyUser/GetObjectById_User",
        contentType: "application/json; charset=utf-8",
        dataType: "json",
        data: param2,
        beforeSend: function () {
...
function modifyFun(param) {
    $.ajax({
        type: "POST",
        url: "/IWantToBuyUser/IWantToBuy_Update",
        contentType: "application/json; charset=utf-8",
        dataType: "json",
        data: param,
        beforeSend: function () {
...
function getJobsList(param) {
    ...
    $.ajax({
        type: "POST",
        url: "/IWantToBuyUser/IWantToSupply_List",
        contentType: "application/json; charset=utf-8",
        dataType: "json",
        data: param1,
        beforeSend: function () {
            ...
            if (!json.Msg.IsSuccess) {
                ...
                for (var i = 0; i < json.List.length; i++) {
                    ...
                    trList += ((json.PageInfo.PageSize * (json.PageInfo.CurrentPage - 1) + i + 1)
                        ...
                        + "</td><td>"
                        + (json.List[i].Status == 0 ? "<span class='span_wytb_hui1' onclick='ChangeStatus(" + json.List[i].Status + "," + json.List[i].IWantToBuyID + "," + json.List[i].Id + ")' enabled='false'>设为中标</span>"
                            : (json.List[i].Status == 1 ? "<span class='span_wytb_lan1' onclick='ChangeStatus(" + json.List[i].Status + "," + json.List[i].IWantToBuyID + "," + json.List[i].Id + ")'>取消中标</span><a style='cursor:pointer;text-decoration:none;width:50px;' href='javascript:void(0)' onclick='ToPay(" + json.List[i].Id + ")'>去支付</a>"
                            : (json.List[i].Status == 4 ? "<span class='span_wytb_lan1' onclick='ChangeStatus(" + json.List[i].Status + "," + json.List[i].IWantToBuyID + "," + json.List[i].Id + ")'>取消中标</span><a style='cursor:pointer;text-decoration:none;width:50px;' href='javascript:void(0)' onclick='ToPay_Continue(" + json.List[i].Id + ")'>继续付款</a>"
                            : (json.List[i].Status == 5 ? ""
                            : (json.List[i].Status == 6 ? "<span class='span_wytb_lan1' onclick='ChangeStatus_Orders(" + json.List[i].Status + "," + json.List[i].IWantToBuyID + "," + json.List[i].Id + ")'>签收</span>&nbsp;<span class='span_wytb_lan1' onclick='DeliverGoodsFun(" + json.List[i].Id + "," + json.List[i].Status + ")'>查看物流</span>"
                            : (json.List[i].Status == 7 ? "<span class='span_wytb_lan1' onclick='DeliverGoodsFun(" + json.List[i].Id + "," + json.List[i].Status + ")'>查看物流</span>"
                            : (json.List[i].Status == 2 ? "<span class='span_wytb_hui1' onclick='ChangeStatus(" + json.List[i].Status + "," + json.List[i].IWantToBuyID + "," + json.List[i].Id + ")' enabled='true'>设为中标</span>"
                            : (json.List[i].Status == 3 ? "<span class='span_wytb'>————</span>"
                            : "<span class='span_wytb_hui1'>状态异常</span>"))))))))
                            + ("&nbsp;<span class='span_wytb_lan1' onclick='onlinetalk(" + json.List[i].ShopId + ")'>在线咨询</span>")
                        + "</td></tr>"
                    );
                }
                tbody.html(trList);
...
function Get_PageInfo(param) {
    ...
    $.ajax({
        type: "POST",
        async: false,
        url: "/IWantToBuyUser/Get_PageInfo_IWantToSupply_List",
        contentType: "application/json; charset=utf-8",
        dataType: "json",
        data: paramJson,
        success: function (data) {
...
function ChangeStatus_Orders(status, iWantToBuyId, Id) {
    var param = getParamUpdateStatus(status, iWantToBuyId, Id);
    param = JSON.stringify(param)
    $.ajax({
        type: "POST",
        url: "/IWantToBuyUser/IWantToSupply_UpdateStatus7",
        contentType: "application/json; charset=utf-8",
        dataType: "json",
        data: param,
        beforeSend: function () {
...
function DeliverGoodsFun(supplyId, status) {
    ...
    $.ajax({
        type: "POST",
        url: "/IWantToBuyUser/GetObjectById_Supply_DeliverGoods",
        contentType: "application/json; charset=utf-8",
        dataType: "json",
        data: param2,
        beforeSend: function () {
    ...
    art.dialog({
        ...
        content: '<table id="table_AddJob" style="margin:0px auto;">'
                ...
                + '<tr>'
                    + '<td>物流公司1：</td>'
                    + '<td>'
                            + '<select id="logisticsType" class="inputTitle" disabled="disabled"  style="margin-left:0px;">'
                            + '</select>'
                    + '</td>'
                + '</tr>'
                + '<tr>'
                    + '<td>物流编号：</td>'
                    + '<td>'
                        + '<input id="logisticsNum" type="text" class="inputTitle" disabled="disabled"  value="' + json.Model.LogisticsNum + '" />'
                    + '</td>'
                + '</tr>'
                + '<tr>'
                    + '<td>物流备注：</td>'
                    + '<td>'
                        + '<textarea rows="3" cols="40" disabled="disabled"  id="logisticsDes" >' + json.Model.LogisticsDes + '</textarea>'
                    + '</td>'
                + '</tr>'
                + '</table>',
        lock: true,
...
/*去支付*/
function ToPay(Id) {
    AddIWantToBuyOrdersFun(Id);
    //$.dialog.confirm("确定去支付？", function () {
    //    location.href = "/Order/Pay?orderIds=" + Id + "&paytype=7&targetid=" + Id, "_blank";
    //});
}

/*继续支付*/
function ToPay_Continue(Id) {
  
    top.location.href = "/Order/Pay?orderIds=" + Id + "&paytype=7&targetid=" + Id, "_blank";
  
}

function AddIWantToBuyOrdersFun(Id) {

    var param1 = getParamAddOrders(Id);
    var param2 = JSON.stringify(param1)

    $.ajax({
        type: "POST",
        url: "/IWantToBuyUser/IWantToBuyOrderAdd",
        contentType: "application/json; charset=utf-8",
        dataType: "json",
        data: param2,
        beforeSend: function () {
        ...
        success: function (data) {
            ...
            if (json.Msg.IsSuccess) {
                ...
                top.location.href = "/Order/Pay?orderIds=" + json.Model.Id + "&paytype=7&targetid=" + Id, "_blank";
