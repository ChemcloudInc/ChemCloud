...
(function ($) {
    function uploadfile(target, opts) {
        ...
        var ImageHideFile = $(target).find("input:hidden").eq(0); //隐藏域。
        if (imgupfile.val() == "") {
            $.dialog.errorTips("请选择要上传的PDF！");
            return false;
        }
        else {
            ...
            if (!checkImgType(imgupfile.val())) {
                $.dialog.errorTips("上传格式为pdf", '', 3);
                return false;
            }
            if (!flag) {
                $.dialog.errorTips("上传的pdf不能超过" + opts.maxSize + "M");
                return;
            }
        }
    ...
    $.fn.hishopUpload.defaults = {
        url: '/common/PublicOperation/UploadPicMember',
        displayImgSrc: '',
        title: 'pdf：',
        imageDescript: '建议上传15M以内的pdf',
        imgFieldName: 'Icon',
        view: renderView,
        headerWidth: 2,
        dataWidth: 2,
