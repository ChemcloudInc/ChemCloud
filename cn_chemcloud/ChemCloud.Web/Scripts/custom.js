...
$(document).ready(function () {
    ...
    $('.navbar-nav.navbar-left').find('.dropdown').click(function () {
        ...
        $('.nav-pills').html($(this).find("input").val());
        $("#" + id).parent().addClass("active").siblings().removeClass("active");

        //默认跳转至第一个二级菜单的链接
        $("iframe").attr("src", $(this).find('li').first().children().attr("href"));
        if (!$('.nav-pills').find("li").hasClass("active")) {
            $('.nav-pills').find("li:eq(0)").addClass("active");
        }
    ...
    var id = "";
    $('.navbar-nav .dropdown').on('click', 'li', function () {
        $(this).addClass('current').siblings().removeClass('current');
        id = "a" + $(this).find("a").attr("id");
    });
    ...
    if (myurl != null && myurl.toString().length > 1) {
        ...
        $(".nav.navbar-left ul li a").each(
            function (index, item) {
                ...
                //    $(item).parent().click();
                //}
                if ($(item).attr("href").indexOf(tar) >= 0) {
                    $(item).parent().click();
                }
            }
        );
...
var click1 = function (a) {
    $(a).addClass("active").siblings().removeClass("active");

}

var mouseover1 = function (a) {
    $(a).addClass("hover").siblings().removeClass("hover");
}

var clear = function () {
    $(".nav-pills i").css("background-image", "url('../Images/Seller.png')");
}
