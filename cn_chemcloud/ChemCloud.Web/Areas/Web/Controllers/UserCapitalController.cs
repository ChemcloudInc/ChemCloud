...
namespace ChemCloud.Web.Areas.Web.Controllers
{
    public class UserCapitalController : BaseMemberController
    {
        ...
        public ActionResult ApplyWithDraw()
        {
            ...
            if (fwinfo == null)
            {
                return RedirectToAction("index", "userCenter", new { url = "/MoneyList" });
            }
        ...
        public JsonResult GetPayBankInfo(string bankName, string bankuserName, string bankId)
        {
            ...
            if (fwinfo == null)
            {
                Log.Error("用户：" + base.CurrentUser.Id + "在" + DateTime.Now + "提现时获取自己的提现信息失败.");
                return Json("");
            }
            else
            {
                ...
                {
                    Log.Info("用户：" + base.CurrentUser.Id + "在" + DateTime.Now + "提现时设置提现信息成功.");
                    return Json("ok");
                }
                else
                {
                    Log.Error("用户：" + base.CurrentUser.Id + "在" + DateTime.Now + "提现时设置提现信息失败.");
                    return Json("");
                }
            }
        }
        ...
        public JsonResult ApplyWithDrawSubmit(decimal amount, string pwd)
        {
            ...
            if (fwino == null)
            {
                ...
                return Json("用户" + base.CurrentUser.Id + "在" + DateTime.Now + "时提现 获取提现信息失败.");
            }
            else
            {
                if (!pwd.Equals(fwino.Wallet_PayPassword))
                {
                    return Json("支付密码不对，请重新输入！");
                }
                else
                {
                    if (amount > fwino.Wallet_UserLeftMoney)
                    {
                        return Json("提现金额不能超出可用金额！");
                    }
                    else
                    {
                        //创建提现记录 并且更新用户提现后的钱包
                        ...
                        else
                        {
                            ...
                            return Json("用户" + base.CurrentUser.Id + "在" + DateTime.Now + "时添加提现记录失败.");
                        }
        ...
        /// <returns></returns>
        public JsonResult AddZZinfo(string pwd, string uname, string zmoney)
        {
