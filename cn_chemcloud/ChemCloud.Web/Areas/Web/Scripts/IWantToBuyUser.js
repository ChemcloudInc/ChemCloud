...
function open_ModifyDialog(json) {
    art.dialog({
        ...
        ok: function () {
            ...
            var isTrue1 = getParamModifyVal(json.Model.Model.Status);
            if (isTrue1) {
                var param = getParamAdd();
...
/*------------------------------------------  修改状态 Start  -------------------------------------------*/
function span_UpdateStatusFun(obj, status, currentStatus) {
    var objSpan = $(obj);
    var Id = objSpan.attr("itemid");
    var isTrue = getParamModifyVal(currentStatus);
    if (isTrue) {
        var param = getParamModify(Id);
...
function getJobsList(param) {
    ...
    $.ajax({
        type: "POST",
        url: "/IWantToBuyUser/IWantToBuyList_User",
        contentType: "application/json; charset=utf-8",
        dataType: "json",
        data: param1,
        beforeSend: function () {
            ...
            if (!json.Msg.IsSuccess) {
                ...
                for (var i = 0; i < json.List.length; i++) {
                    ...
                    trList += ((json.PageInfo.PageSize * (json.PageInfo.CurrentPage - 1) + i + 1)
                        ...
                        + "</td><td><span onclick='span_UpdateStatusFun(this,1," + json.List[i].Status + ")' itemId='" + json.List[i].Id + "'>废弃</span> | <span onclick='span_UpdateStatusFun(this,2," + json.List[i].Status + ")' itemId='" + json.List[i].Id + "'>终止</span> | <span onclick='span_ModifyFun(this)' itemId='" + json.List[i].Id + "' status='" + json.List[i].Status + "'>查看修改</span> | <a href='/IWantToBuyUser/Get_SupplyList/" + json.List[i].Id + "'>竞价列表</a></td></tr>"
                    );
                }
                tbody.html(trList);
...
function Get_PageInfo(param) {
    ...
    $.ajax({
        type: "POST",
        async: false,
        url: "/IWantToBuyUser/Get_PageInfo_User",
        contentType: "application/json; charset=utf-8",
        dataType: "json",
        data: paramJson,
        success: function (data) {
...
function getParamSelectVel(param) {
    ...
    if (param.query.ParamInfo.inputEndDate1 < param.query.ParamInfo.inputStartDate1) {
        message = "发布日期的开始时间不能晚于结束时间<br/>";
    }
    if (param.query.ParamInfo.inputEndDate2 < param.query.ParamInfo.inputStartDate2) {
        message = "交付日期的开始时间不能晚于结束时间<br/>";
    }
